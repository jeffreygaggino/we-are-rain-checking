...
Working agreement for AI agents on this service. Language-agnostic; fill in the `<>` placeholders once per project.

`STRUCTURE-go-gin-backend.md` is the kickoff prompt — it scaffolds a new service and is done once the skeleton stands.
**This file governs everything after that.** Where the two overlap on a matter of code, STRUCTURE is authoritative.

## Project specifics (fill these in)

- **This service is called:** `<repo name>` — not its container/pod/deployment alias.
- **Services it talks to:** `<name — what it owns>`.
- **Plans live in:** `<plans/>` — markdown, numbered `NN-` by creation date, one per feature or investigation.
- **Verification gate:** `<format named files>` → `<lint/vet>` → `<build>` → `<test>`.
- **Never run:** `<repo-wide format/codemod commands that rewrite unrelated files>`, `<destructive db/env commands>`,
  and any **deploying** Make target. Check the Makefile before running a target: `make dev` / `make prod` commonly
  build, push **and** SSH to reload a live environment — the local one is usually `make local`. A target named for an
  environment acts on that environment.
...
### Auth on every handler, and no bypasses for convenience

Permission checks go through **one helper**, with the permission constants owned by the same package. Never reintroduce
a fake-user or auth-skip path to make local development work — if local auth is broken, the token flow is the bug to
fix. A bypass that exists for testing eventually ships.
...
### Proxying and passthrough: forward what makes the contract work

When this service sits in front of another:

- **Forward the headers the semantics depend on** — range requests, conditional headers, content type, content length.
  Dropping conditional headers makes `304`s impossible and forces a full re-download on every revisit.
- **Order matters when both sides change.** Forwarding conditional headers must land *before* the upstream starts
  emitting validators, or valid `304`s become gateway errors.
- **Absent cache headers are not neutral.** With no `Cache-Control` at all, caches invent heuristic freshness from the
  last-modified date, so an old file can be served stale for days. `no-cache` means "revalidate before reuse", not
  "don't store" — it's the correctness-preserving default when consumers must always get the current bytes.
- **Prefer one validator that already works** over adding a second mechanism. If the upstream already honours
  `If-Modified-Since`, the blocker is your forwarding, not their missing `ETag`.

### One service owns a resource

A mount, a queue, a filesystem, a third-party credential — exactly one service should own it, and the others go
through that service's API. When ownership moves, remove the old mount and the checks that depended on it, so a stale
handle can only break in one place.

Corollary: keep pure transformation (path resolution, type detection, parsing) separate from I/O checks (existence,
reachability). The pure parts stay wherever they're convenient; the I/O belongs to the owner.

### Cross-service contracts: state the axis, prefer content over names

When two services dedup, reconcile, or adopt each other's records, write down **exactly what makes two records the
same** and keep it in the plan.

- **Dedup on content, not filename.** A file re-exported under the same name is a different artifact; name-keyed dedup
  silently adopts the stale one. If the contract is name-keyed and you can't change it, record it as a known
  correctness risk rather than forgetting it.
- **Scope reconciliation by the axis that actually distinguishes records** (the account/tenant), not an incidental
  field like source or client. A search filtered by the wrong axis makes another producer's records invisible and
  every conflict unresolvable.
- **On a duplicate conflict, adopt the existing record** rather than retrying the write.
- **Beware retry resets on boot.** A startup sweep that re-eligibles every failed row will loop a permanently
  unresolvable one fail→eligible→fail on every restart.
...
### Reading logs and metrics honestly

- **Request latency that includes writing the response body is transfer time, not processing time.** A 20-second entry
  for a 120 MB range and a 22-millisecond entry for a 38 kB read describe the same throughput. No log line means
  anything without the payload size beside it.
- Compute the implied rate before concluding "slow storage" — it's usually the chain, not the disk.
- Record what you got **wrong** in the investigation alongside what you found, so it isn't re-derived next time.
...
- **Keep:** a non-obvious ordering, a lock's scope, a constant's units, a rule that looks wrong but isn't, a gotcha
  that will bite ("this must land before the upstream sends validators").
...
- One line where one line does. A pointer to `plans/NN §x` replaces the paragraph you were about to write.
